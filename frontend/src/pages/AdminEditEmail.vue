...
<p class="mb-4 text-gray-700">
  📧 อีเมลปัจจุบัน: <span class="font-medium">{{ currentEmail }}</span>
    ...
    <div v-if="successMessage" class="bg-green-100 text-green-800 p-2 mb-4 rounded">
      {{ successMessage }}
    </div>
...
const fetchEmail = async () => {
  try {
    const res = await axios.get('http://localhost:8080/api/admin/settings/email', { withCredentials: true })
...
const updateEmail = async () => {
  try {
    await axios.post('http://localhost:8080/api/admin/settings/update-email', { email: newEmail.value }
