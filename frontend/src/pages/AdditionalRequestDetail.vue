<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50">
      ...
      <div class="max-w-6xl mx-auto px-4 py-4 sm:px-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800">Request Details</h1>
          </div>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8 sm:px-6">
      ...
      <div v-else-if="!request" class="text-center py-20">
        <div class="bg-red-50 border border-red-200 rounded-xl p-8 max-w-md mx-auto">
          ...
          <h3 class="text-lg font-semibold text-red-800 mb-2">Request Not Found</h3>
          <p class="text-red-600">The requested item could not be found or may have been removed.</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div :class="[
            'p-6 text-center',
            request.status === 'PENDING' ? 'bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-200' :
            request.status === 'REJECTED' ? 'bg-gradient-to-r from-red-50 to-pink-50 border-b border-red-200' :
            request.status === 'in_shelf' ? 'bg-gradient-to-r from-emerald-50 to-green-50 border-b border-emerald-200' :
            request.status === 'ordered' ? 'bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-200' :
            request.status === 'popular_request' ? 'bg-gradient-to-r from-purple-50 to-indigo-50 border-b border-purple-200' :
            ...
            <div class="flex items-center justify-center gap-3 mb-2">
              <div :class="[
                ...
                request.status === 'in_shelf' ? 'bg-emerald-500' :
                request.status === 'ordered' ? 'bg-blue-500 animate-pulse' :
                request.status === 'popular_request' ? 'bg-purple-500' :
                'bg-slate-400'
              ]"></div>
              <h2 :class="[
                'text-xl font-bold',
                ...
                request.status === 'in_shelf' ? 'text-emerald-700' :
                request.status === 'ordered' ? 'text-blue-700' :
                request.status === 'popular_request' ? 'text-purple-700' :
                'text-slate-700'
              ]">{{ displayStatus(request.status) }}</h2>
            </div>
            <p class="text-sm opacity-75">{{ getStatusDescription(request.status) }}</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div class="p-6 space-y-6">
            ...
            <div class="pb-4 border-b border-slate-100">
              <h4 class="text-2xl font-bold text-slate-800 mb-2">{{ request.bookTitle }}</h4>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-4">
                <div v-if="request.publisher">
                  <label class="block text-sm font-medium text-slate-700 mb-1">Publisher</label>
                  <p class="text-slate-600 bg-slate-50 rounded-lg px-3 py-2">{{ request.publisher }}</p>
                </div>

                <div v-if="request.isbn">
                  <label class="block text-sm font-medium text-slate-700 mb-1">ISBN</label>
                  <p class="text-slate-600 bg-slate-50 rounded-lg px-3 py-2 font-mono">{{ request.isbn }}</p>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div v-if="request.price">
                  ...
                  <p class="text-slate-600 bg-slate-50 rounded-lg px-3 py-2">
                    <span class="text-lg font-semibold ">{{ request.price }} ฿</span>
                  </p>
                </div>
                <div v-if="request.description">
                  <label class="block text-sm font-medium text-slate-700 mb-1">description</label>
                  <p class="text-slate-600 bg-slate-50 rounded-lg px-3 py-2">
                    <span class="text-lg font-semibold ">{{ request.description }} ฿</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div v-if="request.reason" class="pt-4 border-t border-slate-100">
              <label class="block text-sm font-medium text-slate-700 mb-2">Request Reason</label>
              <div class="bg-slate-50 rounded-lg p-4">
                <p class="text-slate-600 whitespace-pre-line leading-relaxed">{{ request.reason }}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <div class="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2">
                    ...
                    <span class="text-slate-600">{{ request.requestedBy || 'N/A' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <div class="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2">
                    ...
                    <span class="text-slate-600">{{ formatDate(request.requestDate) }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <!-- Action Buttons -->
        <div v-if="request && request.status !== 'REJECTED'" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
    ...
    <div v-if="showApproveDialog" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
        <div class="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white rounded-t-2xl">
          ...
          <p class="text-indigo-100 text-sm mt-1">
            {{ request.status === 'PENDING' ? 'Choose the final status for this request' : 'Select a new status for this request' }}
          </p>
        </div>

        <div class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">Select Status</label>
            <select
              v-model="finalStatus"
